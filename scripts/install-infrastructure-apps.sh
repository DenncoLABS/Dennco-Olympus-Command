...
if [ "${EUID:-$(id -u)}" -ne 0 ]; then
  echo "Run as root: sudo $0 <zabbix|agent-dvr|rc2|freepbx|nethserver8|proxmox8|all>" >&2
  exit 1
fi
...
case "$APP" in
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: sudo $0 <zabbix|agent-dvr|rc2|freepbx|nethserver8|proxmox8|all>" >&2
    exit 2
    ;;
esac
