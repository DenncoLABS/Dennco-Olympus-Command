...
export const TileRuntimeCard: React.FC<TileRuntimeCardProps> = ({ tileId, selected = false, onSelect, onFocusTile }) => {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      onDoubleClick={onFocusTile}
      onKeyDown={handleKeyDown}
      aria-pressed={selected}
      title="Click to select. Double-click or press F to focus."
    ...
    >
      ...
      <div className="relative mt-3 flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-[radial-gradient(circle_at_center,rgba(34,211,238,0.14),transparent_44%),linear-gradient(135deg,rgba(255,255,255,0.04),transparent)] text-center">
        ...
        <div className="relative px-4">
          ...
          {selected ? <div className="mt-3 text-[9px] uppercase tracking-[0.16em] text-cyan-200">Selected · Double-click or press F to focus</div> : null}
        </div>
      </div>
    </button>
  );
