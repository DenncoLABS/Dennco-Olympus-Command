...
const architectureNodes = [
  ...
  ['Desk Widget Registry', 'Monitor widgets, saved layouts, Earth staging placeholders, future overlay windows'],
...
function ServicesView() {
  return <TableView title="Debian / Olympus Services" intro="Service status placeholders for the OS Desk. Live systemd/API checks can be wired into this panel later." headers={['Service', 'Role', 'State']} rows={serviceRows} />;
}

function PackagesView() {
  return <TableView title="Debian Package Controls" intro="Package and workflow placeholders for apt publish, reinstall verification, and future GUI controls." headers={['Package / Area', 'Role', 'State']} rows={packageRows} />;
}

function SettingsView({ dock, setDock, height, setHeight }: { dock: DockPlacement; setDock: (dock: DockPlacement) => void; height: number; setHeight: (height: number) => void }) {
  return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">Desk / Dock Settings</h3><p className="mt-2 text-sm text-white/55">Local Desk settings are saved in browser localStorage. These controls stage the future GNOME-style shell preferences.</p><div className="mt-4 grid gap-3 md:grid-cols-2"><StatusCard title="Dock Placement" value={dock} status="saved locally" /><StatusCard title="Desk Height" value={`${height}px`} status="saved locally" /><StatusCard title="Earth Screen Behavior" value="Desk reserves layout space and pushes map upward" status="active" /><StatusCard title="GNOME Integration" value="desktop file, autostart, icon package, kiosk mode" status="planned" /></div><div className="mt-4 flex gap-2 text-[10px] uppercase tracking-[0.14em]"><button onClick={() => setDock('left')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Left</button><button onClick={() => setDock('center')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Center</button><button onClick={() => setDock('right')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Right</button><button onClick={() => setHeight(240)} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Reset Height</button></div></div>;
}

function ModuleView({ view }: { view: DeskView }) {
  return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">{view} workspace</h3><p className="mt-3 text-sm text-white/60">Desk workspace for {view}. Earth/map screen remains unchanged until a widget is intentionally placed onto it.</p><div className="mt-4 grid gap-3 md:grid-cols-2"><StatusCard title="Desk App Surface" value={`${view} opens inside Olympus Desk`} status="active" /><StatusCard title="Earth Screen" value="No automatic map takeover" status="protected" /><StatusCard title="Future Widgets" value="Draggable windows can later pin to Earth" status="planned" /><StatusCard title="Local State" value="Desk view and dock settings saved locally" status="active" /></div></div>;
}

function TableView({ title, intro, headers, rows }: { title: string; intro: string; headers: readonly string[]; rows: readonly (readonly string[])[] }) {
  return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">{title}</h3><p className="mt-2 text-sm text-white/55">{intro}</p><div className="mt-4 overflow-hidden border border-white/10"><table className="w-full text-left text-xs"><thead className="bg-white/[0.04] text-white/35 uppercase tracking-[0.14em]"><tr>{headers.map((header) => <th key={header} className="p-2">{header}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row[0]} className="border-t border-white/5 text-white/60 hover:bg-cyan-300/5">{row.map((cell) => <td key={cell} className="p-2">{cell}</td>)}</tr>)}</tbody></table></div></div>;
}

function StatusCard({ title, value, status }: { title: string; value: string; status: string }) {
  return <div className="border border-white/10 bg-white/[0.03] p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-white/35">{title}</div><div className="mt-1 break-all font-mono text-white">{value}</div><div className="mt-2 text-[9px] uppercase tracking-[0.14em] text-cyan-300">{status}</div></div>;
}
