...
const deskItems: DeskItem[] = [
  ...
  { id: 'intelmaps', label: 'Intel Maps', icon: '▤', view: 'intelmaps', module: 'intelmaps', group: 'Operational', status: 'active', description: 'Deploys the Intel Maps toolbar and map workspace. Flight, Maritime, Monitor, DOT, and Cyber maps open from that app.' },
  { id: 'monitor', label: 'Monitor', icon: '◉', view: 'monitor', module: 'monitor', group: 'Operational', status: 'active', description: 'Monitor Desk widgets and intelligence cards. Map view opens from Intel Maps.' },
...
function CoreView() { return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">Core System</h3><p className="mt-3 text-sm text-white/60">Olympus Core is the OS control surface for Debian/GNOME services, tile screens, package state, data folders, apps, and command modules.</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs"><StatusCard title="Debian Host" value="apt + systemd underneath Olympus" status="active" /><StatusCard title="Tile Screens" value="core route and monitor viewing surface" status="active" /><StatusCard title="Intel Maps App" value="single Dock launcher for all maps" status="active" /><StatusCard title="Systemd Service" value="dennco-olympus-command" status="active" /><StatusCard title="CAD Service" value="olympus-cad · :5050/health" status="active" /><StatusCard title="GNOME Launcher" value="/usr/share/applications/olympus-command.desktop" status="planned" /></div></div>; }
function IntelMapsDeskView() { return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">Intel Maps</h3><p className="mt-3 text-sm text-white/60">The Intel Maps Dock button deploys the map workspace bar above the Desk. Flight, Maritime, Monitor, DOT, and Cyber are no longer separate Dock launchers; they open from the Intel Maps dropdown.</p><div className="mt-4 grid gap-3 md:grid-cols-2"><StatusCard title="Workspace Bar" value="Multi-Purpose Earth Workspace" status="deployed above" /><StatusCard title="Map Dropdown" value="Flight · Maritime · Monitor · DOT · Cyber" status="active" /><StatusCard title="Close App" value="closes all map windows and returns to blank tile screens" status="active" /><StatusCard title="Dock Role" value="single launcher for map app" status="active" /></div></div>; }
...
function SettingsView({ dock, setDock, height, setHeight }: { dock: DockPlacement; setDock: (dock: DockPlacement) => void; height: number; setHeight: (height: number) => void }) { return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">Desk / Dock Settings</h3><p className="mt-2 text-sm text-white/55">Local Desk settings are saved in browser localStorage. These controls stage the future GNOME-style shell preferences.</p><div className="mt-4 grid gap-3 md:grid-cols-2"><StatusCard title="Dock Placement" value={dock} status="saved locally" /><StatusCard title="Desk Height" value={`${height}px`} status="saved locally" /><StatusCard title="Tile Screen Behavior" value="Core route shows blank tile screens. Intel Maps opens the map toolbar." status="active" /><StatusCard title="GNOME Integration" value="desktop file, autostart, icon package, kiosk mode" status="planned" /></div><div className="mt-4 flex gap-2 text-[10px] uppercase tracking-[0.14em]"><button onClick={() => setDock('left')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Left</button><button onClick={() => setDock('center')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Center</button><button onClick={() => setDock('right')} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Dock Right</button><button onClick={() => setHeight(240)} className="border border-white/10 px-2 py-1 text-white/55 hover:border-cyan-300/40 hover:text-cyan-200">Reset Height</button></div></div>; }
