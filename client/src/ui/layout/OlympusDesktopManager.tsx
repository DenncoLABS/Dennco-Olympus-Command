...
const HEIGHT_KEY = 'olympus.desktop.height.v1';
const DOCK_KEY = 'olympus.desktop.dock.v1';
const WINDOWS_KEY = 'olympus.desktop.windows.v1';
...
function defaultWindow(view: DeskView, offset = 0): DeskWindow {
  const item = deskItems.find((entry) => entry.view === view);
  return { id: view, view, title: item?.label || view, x: 24 + offset * 30, y: 18 + offset * 18, w: view === 'monitor' ? 720 : 540, h: view === 'monitor' ? 320 : 260, z: Date.now() + offset };
}

function readWindows(): DeskWindow[] {
  try {
    const raw = localStorage.getItem(WINDOWS_KEY);
    if (!raw) return [defaultWindow('core')];
    ...
    return valid.length ? valid : [defaultWindow('core')];
  } catch {
    return [defaultWindow('core')];
  }
}
...
export const OlympusDesktopManager: React.FC = () => {
  ...
  const [deskHeight, setDeskHeight] = useState(() => clampHeight(readNumber(HEIGHT_KEY, 276)));
    ...
    <section className="relative z-[4200] w-full shrink-0 border-t border-cyan-300/25 bg-black/90 font-mono shadow-[0_-16px_40px_rgba(0,0,0,0.82)]" style={{ height: open ? deskHeight : 44 }}>
      ...
      <div className="flex h-full flex-col overflow-hidden">
        <div className="flex h-11 items-center justify-between border-b border-white/10 px-4 pt-2">
          <div><div className="text-[10px] uppercase tracking-[0.26em] text-cyan-300">Olympus Desk</div><div className="text-[9px] uppercase tracking-[0.16em] text-white/40">Desktop mode · windows stay inside the Desk</div></div>
...
function ArchitectureView() { return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">Architecture</h3><p className="mt-3 text-sm text-white/60">Debian → GNOME → Olympus Core → Earth screen → Desk → Dock → Desktop windows.</p></div>; }
