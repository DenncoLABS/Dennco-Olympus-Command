...
import { GnomePanel, PackagesPanel, ServicesPanel } from './OlympusOsPanels';

type DeskView = 'core' | 'apps' | 'files' | 'architecture' | 'terminal' | 'services' | 'packages' | 'gnome' | 'settings';
...
const HEIGHT_KEY = 'olympus.desktop.height.clean.v3';
const DOCK_KEY = 'olympus.desktop.dock.clean.v3';
const VIEW_KEY = 'olympus.desktop.view.clean.v3';
const ACTIVE_APP_KEY = 'olympus.desktop.activeApp.clean.v3';
const HOVER_OPEN_KEY = 'olympus.desktop.hoverOpen.clean.v3';
...
const dockItems: DeskItem[] = [
  ...
  { id: 'gnome', label: 'GNOME', icon: '◌', view: 'gnome' },
...
function readNumber(key: string, fallback: number) { const raw = Number(localStorage.getItem(key)); return Number.isFinite(raw) ? raw : fallback; }
function clampHeight(value: number) { return Math.max(132, Math.min(value, Math.floor(window.innerHeight * 0.74))); }
function readDock(): DockPlacement { const raw = localStorage.getItem(DOCK_KEY); return raw === 'full' || raw === 'left' || raw === 'right' || raw === 'center' ? raw : 'center'; }
function readView(): DeskView { const raw = localStorage.getItem(VIEW_KEY) as DeskView | null; return dockItems.some((item) => item.view === raw) ? raw as DeskView : 'core'; }
function readActiveApp(): ActiveDeskApp { const raw = localStorage.getItem(ACTIVE_APP_KEY); return raw === 'intel-maps' || raw === 'cad' ? raw : null; }
function readHoverOpen() { return localStorage.getItem(HOVER_OPEN_KEY) === 'true'; }

const dockJustify: Record<DockPlacement, string> = { full: 'justify-center', left: 'justify-start', center: 'justify-center', right: 'justify-end' };
...
export const OlympusDesktopManagerClean: React.FC = () => {
  ...
  useEffect(() => { if (activeDeskApp) localStorage.setItem(ACTIVE_APP_KEY, activeDeskApp); else localStorage.removeItem(ACTIVE_APP_KEY); }, [activeDeskApp]);

  const startDeskResize = (event: React.PointerEvent<HTMLDivElement>) => { deskDrag.current = { id: event.pointerId, y: event.clientY, h: deskHeight }; event.currentTarget.setPointerCapture(event.pointerId); };
  const moveDeskResize = (event: React.PointerEvent<HTMLDivElement>) => { const drag = deskDrag.current; if (!drag || drag.id !== event.pointerId) return; setDeskHeight(clampHeight(drag.h + (drag.y - event.clientY))); };
  const endDeskResize = (event: React.PointerEvent<HTMLDivElement>) => { if (!deskDrag.current || deskDrag.current.id !== event.pointerId) return; deskDrag.current = null; try { event.currentTarget.releasePointerCapture(event.pointerId); } catch { /* capture already released */ } };

  const openView = (view: DeskView) => { setOpen(true); setActiveView(view); if (view !== 'apps') setActiveDeskApp(null); };
  ...
  return (
    <section className="relative z-[4200] w-full shrink-0 border-t border-cyan-300/25 bg-black/90 font-mono shadow-[0_-16px_40px_rgba(0,0,0,0.82)] transition-[height] duration-200" style={{ height: visibleHeight }} onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}>
      ...
      {effectiveOpen && <div className="flex h-full flex-col overflow-hidden">
        <div className="flex h-11 items-center justify-between border-b border-white/10 px-4 pt-2"><div><div className="text-[10px] uppercase tracking-[0.26em] text-cyan-300">Olympus Desk</div><div className="text-[9px] uppercase tracking-[0.16em] text-white/40">OS workspace · GNOME, packages, services, maps, and CAD</div></div><div className="flex items-center gap-2 text-[9px] uppercase tracking-[0.14em] text-white/45">{activeDeskApp && <span className="text-cyan-200">App Bar: {activeDeskApp === 'intel-maps' ? 'Intel Maps' : 'CAD'}</span>}<button onClick={() => setDock('full')} className={`border px-2 py-1 ${dock === 'full' ? 'border-cyan-300/60 text-cyan-200' : 'border-white/10 hover:border-cyan-300/40'}`}>Full</button><button onClick={() => setDock('left')} className={`border px-2 py-1 ${dock === 'left' ? 'border-cyan-300/60 text-cyan-200' : 'border-white/10 hover:border-cyan-300/40'}`}>Dock Left</button><button onClick={() => setDock('center')} className={`border px-2 py-1 ${dock === 'center' ? 'border-cyan-300/60 text-cyan-200' : 'border-white/10 hover:border-cyan-300/40'}`}>Dock Center</button><button onClick={() => setDock('right')} className={`border px-2 py-1 ${dock === 'right' ? 'border-cyan-300/60 text-cyan-200' : 'border-white/10 hover:border-cyan-300/40'}`}>Dock Right</button><button onClick={() => setHoverOpen((value) => !value)} className={`border px-2 py-1 ${hoverOpen ? 'border-emerald-300/60 text-emerald-200' : 'border-white/10 hover:border-emerald-300/40'}`}>Hover Open</button><button onClick={() => { setOpen(false); setHovering(false); }} className="border border-white/10 px-2 py-1 text-cyan-200 hover:border-cyan-300/60">Hide</button></div></div>
...
function DeskContent({ view, activeDeskApp, activeModule, mapApps, onOpenIntelMaps, onOpenCad, onSelectMap, onCloseAppBar }: { view: DeskView; activeDeskApp: ActiveDeskApp; activeModule: ActiveModule; mapApps: Array<{ id: ActiveModule; label: string; detail: string }>; onOpenIntelMaps: () => void; onOpenCad: () => void; onSelectMap: (module: ActiveModule) => void; onCloseAppBar: () => void }) {
  ...
  if (view === 'services') return <ServicesPanel />;
  if (view === 'packages') return <PackagesPanel />;
  if (view === 'gnome') return <GnomePanel />;
  if (view === 'settings') return <SettingsView />;
  return <CoreView onOpenAdmin={() => onSelectMap('admin')} />;
}

function SectionTitle({ title, subtitle }: { title: string; subtitle?: string }) { return <div><h3 className="text-cyan-200 uppercase tracking-[0.18em] text-sm">{title}</h3>{subtitle && <p className="mt-2 text-sm text-white/55">{subtitle}</p>}</div>; }
function CoreView({ onOpenAdmin }: { onOpenAdmin: () => void }) { return <div className="h-full overflow-auto"><SectionTitle title="Core" subtitle="Utilities, files, packages, services, admin, and desktop controls live here." /><div className="mt-4 grid grid-cols-2 lg:grid-cols-5 gap-3 text-sm"><CoreCard title="Files" detail="File browser staging." /><CoreCard title="Packages" detail="Package and source controls." /><CoreCard title="Services" detail="Service status and controls." /><CoreCard title="GNOME" detail="Desktop launcher and autostart." /><button onClick={onOpenAdmin} className="text-left border border-white/10 bg-white/[0.03] p-3 hover:border-cyan-300/40"><div className="font-bold text-white">Admin</div><div className="mt-1 text-xs text-white/40">Open Admin screen.</div></button></div><div className="mt-5"><MonitorDeskWidgets /></div></div>; }
...
function ArchitectureView() { return <div><SectionTitle title="Architecture" subtitle="Debian → GNOME → Olympus Core → Earth screen → Desk → Dock → Apps." /><div className="mt-3 text-xs leading-6 text-white/60"><div>Apps → Intel Maps / CAD</div><div>Intel Maps → Flight / Maritime / Monitor / DOT</div><div>Core → Files / Packages / Services / GNOME / Admin</div></div></div>; }
