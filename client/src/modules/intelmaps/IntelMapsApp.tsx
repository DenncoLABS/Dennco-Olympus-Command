...
function defaultCustomMapTools(index: number): CustomMapTools { return { title: `New Intel Map ${index}`, baseLayer: 'Dark tactical base', overlay: 'None', drawingMode: 'Point markers', symbology: 'Cyan operational', notes: '', mapPurpose: 'Saved operational view', assetFilters: 'All visible assets', layerFilters: 'Selected layers only', feedFilters: 'Manual selection', savedStateMode: 'Save current state as a new Intel Map', isMaster: false }; }
function masterIntelMapTools(): CustomMapTools { return { title: 'Master Intel Map', baseLayer: 'Dark tactical base', overlay: 'All operational layers', drawingMode: 'Asset builder', symbology: 'Cyan operational', notes: 'Master builder map for creating assets, filtering all available layers/assets/feeds, and saving the current operating state into new Intel Maps.', mapPurpose: 'Master builder and state template', assetFilters: 'Aircraft, vessels, vehicles, radar, airports, ports, cameras, towers, infrastructure, custom assets', layerFilters: 'Flights, Maritime, DOT, Monitor, Cyber, Weather, Radar, Static Infrastructure', feedFilters: 'All available live and static feeds', savedStateMode: 'Build state here, then save state into a new Intel Map', isMaster: true }; }
...
export const IntelMapsApp: React.FC = () => {
  ...
  const openCustomMapBuilder = (tools: CustomMapTools, dimensions?: { width: number; height: number }) => {
    const index = nextCustomMapId();
    const id = `${tools.isMaster ? 'master-intel-map' : 'custom'}-${Date.now()}-${index}`;
    const z = nextZ + 1;
    setNextZ(z + 1);
    setMapsMenuOpen(false);
    setWindows((current) => { const next = [...current, clampWindow({ id, kind: 'custom', title: tools.title, x: 46 + current.length * 24, y: 96 + current.length * 20, width: dimensions?.width || 820, height: dimensions?.height || 520, z, minimized: false, tools, instance: defaultInstance('custom') })]; writeWindows(next); return next; });
  };

  const openNewMapBuilder = () => openCustomMapBuilder(defaultCustomMapTools(nextCustomMapId()));
  const openMasterIntelMap = () => openCustomMapBuilder(masterIntelMapTools(), { width: 980, height: 620 });
  ...
  const saveCustomMap = async (id: string, tools: CustomMapTools) => {
    const payload = { name: tools.title, title: tools.title, description: tools.mapPurpose || '', tools, layers: [tools.baseLayer, tools.overlay, tools.layerFilters].filter((item) => item && item !== 'None'), feeds: [tools.feedFilters].filter(Boolean), assetFilters: { query: tools.assetFilters || '', assetTypes: [], assetIds: [], statuses: [], sources: [], tags: [] }, trackedAssetIds: [], movement: { showTrails: true, trailHours: 24, followSelectedAsset: false }, notes: tools.notes };
...
function CustomMapBuilder({ win, onUpdate, onSave }: { win: WorkspaceWindow; onUpdate: (patch: Partial<WorkspaceWindow>) => void; onSave: (id: string, tools: CustomMapTools) => void }) {
  ...
  return <div className="grid h-full grid-cols-[330px_1fr] text-white/70"><aside className="overflow-auto border-r border-cyan-300/15 bg-[#030812] p-3 text-xs"><div className="text-[10px] uppercase tracking-[0.2em] text-emerald-300">{tools.isMaster ? 'Master Intel Map' : 'Map Building Tools'}</div><div className="mt-1 text-[9px] leading-relaxed text-white/40">{tools.isMaster ? 'Use this master map to build map states, filter assets/layers/feeds, and save those states into new Intel Maps.' : 'Build and save a new Intel Map view.'}</div><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Map Name</label><input value={tools.title} onChange={(event) => setTool({ title: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Purpose</label><input value={tools.mapPurpose || ''} onChange={(event) => setTool({ mapPurpose: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Base Layer</label><select value={tools.baseLayer} onChange={(event) => setTool({ baseLayer: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100"><option>Dark tactical base</option><option>Street operations base</option><option>Satellite planning base</option><option>Maritime chart base</option></select><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Layer Filters</label><textarea value={tools.layerFilters || ''} onChange={(event) => setTool({ layerFilters: event.target.value })} className="mt-1 h-16 w-full resize-none border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Asset Filters</label><textarea value={tools.assetFilters || ''} onChange={(event) => setTool({ assetFilters: event.target.value })} className="mt-1 h-16 w-full resize-none border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Feed Filters</label><textarea value={tools.feedFilters || ''} onChange={(event) => setTool({ feedFilters: event.target.value })} className="mt-1 h-14 w-full resize-none border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">State Save Mode</label><input value={tools.savedStateMode || ''} onChange={(event) => setTool({ savedStateMode: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Drawing Tool</label><select value={tools.drawingMode} onChange={(event) => setTool({ drawingMode: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100"><option>Asset builder</option><option>Point markers</option><option>Line routes</option><option>Polygon zones</option><option>Radius rings</option><option>Text labels</option></select><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Symbology</label><select value={tools.symbology} onChange={(event) => setTool({ symbology: event.target.value })} className="mt-1 w-full border border-white/10 bg-black/60 px-2 py-1 text-cyan-100"><option>Cyan operational</option><option>Amber caution</option><option>Red emergency</option><option>Green logistics</option></select><label className="mt-3 block text-[9px] uppercase tracking-[0.14em] text-white/35">Notes</label><textarea value={tools.notes} onChange={(event) => setTool({ notes: event.target.value })} className="mt-1 h-20 w-full resize-none border border-white/10 bg-black/60 px-2 py-1 text-cyan-100 outline-none focus:border-cyan-300/50" /><button onClick={() => onSave(win.id, tools)} className="mt-3 w-full rounded border border-emerald-300/35 bg-emerald-300/10 px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-emerald-100 hover:bg-emerald-300/15">{tools.isMaster ? 'Save Master State / New Intel Map' : 'Save to Intel Maps Folder'}</button><div className="mt-2 break-all text-[9px] leading-relaxed text-white/35">{win.savedPath || INTEL_MAPS_FOLDER}</div></aside><main className="relative overflow-hidden bg-[#020617]"><div className="absolute inset-0 opacity-30 bg-[linear-gradient(rgba(34,211,238,.08)_1px,transparent_1px),linear-gradient(90deg,rgba(34,211,238,.08)_1px,transparent_1px)] bg-[size:34px_34px]" /><div className="absolute left-6 top-5 rounded border border-cyan-300/20 bg-black/65 px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-cyan-200">{tools.title}</div>{tools.isMaster && <div className="absolute right-6 top-5 rounded border border-emerald-300/25 bg-emerald-300/10 px-3 py-2 text-[9px] uppercase tracking-[0.14em] text-emerald-200">Master Builder State</div>}<div className="absolute left-6 top-20 max-w-xl rounded border border-white/10 bg-black/55 p-4 text-[10px] leading-relaxed text-white/50"><div className="text-cyan-300 uppercase tracking-[0.18em]">Map State Model</div><div className="mt-2">This map represents the builder state used to create new Intel Maps. Add assets, filter all layers/assets/feeds, then save the current state into a new Intel Map view.</div></div><div className="absolute bottom-5 left-6 right-6 grid grid-cols-4 gap-2 text-[9px] uppercase tracking-[0.12em] text-white/45"><div className="border border-white/10 bg-black/55 p-2"><div className="text-cyan-300">Assets</div>{tools.assetFilters}</div><div className="border border-white/10 bg-black/55 p-2"><div className="text-cyan-300">Layers</div>{tools.layerFilters}</div><div className="border border-white/10 bg-black/55 p-2"><div className="text-cyan-300">Feeds</div>{tools.feedFilters}</div><div className="border border-white/10 bg-black/55 p-2"><div className="text-cyan-300">Save State</div>{tools.savedStateMode}</div></div></main></div>;
}
