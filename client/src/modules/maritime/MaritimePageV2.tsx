...
function popupTitle(popup: MaritimePopup) {
  return popup.type === 'node' ? popup.item.label : popup.item.name;
}
...
export const MaritimePage: React.FC = () => {
  ...
  const filteredVessels = useMemo(() => {
    return vessels.filter((v) => {
      ...
      const moored = v.navigationalStatus === 1 || v.navigationalStatus === 5;
  ...
  return (
    <div className="absolute inset-0 bg-intel-bg overflow-hidden flex flex-col">
      <MaritimeToolbar totalCount={vessels.length} filteredCount={filteredVessels.length} portCount={ports.features.length} installationCount={installations.features.length} />
      ...
      <div className="absolute inset-x-0 bottom-8 h-full bg-intel-panel pointer-events-auto z-0" style={{ top: '56px' }}>
        <Map
          ...
          mapStyle={mapStyle}
          styleDiffing={false}
          interactiveLayerIds={osintDrawerOpen ? [] : ['vessel-points', 'maritime-node-points', 'maritime-port-points', 'maritime-installation-points']}
          onClick={onClick}
        ...
        >
          ...
          {!osintDrawerOpen && showMaritimeNodes && (
            <>
              <Source id="active-maritime-zones" type="geojson" data={zones}><Layer id="active-maritime-zone-fill" type="fill" paint={{ 'fill-color': '#ffffff', 'fill-opacity': 0.035 }} /><Layer id="active-maritime-zone-ring" type="line" paint={{ 'line-color': '#ffffff', 'line-opacity': 0.85, 'line-width': 2 }} /></Source>
              <Source id="active-maritime-pulse" type="geojson" data={pulses}><Layer id="active-maritime-pulse-ring" type="line" paint={{ 'line-color': '#ffffff', 'line-opacity': 0.45, 'line-width': 1.5, 'line-blur': 1 }} /></Source>
              <Source id="active-maritime-sweeps" type="geojson" data={sweeps}><Layer id="active-maritime-sweep-line" type="line" paint={{ 'line-color': '#ffffff', 'line-opacity': 0.95, 'line-width': 2, 'line-blur': 1 }} /></Source>
              <Source id="maritime-nodes" type="geojson" data={nodes}><Layer id="maritime-node-points" type="circle" paint={{ 'circle-radius': ['case', ['boolean', ['get', 'active'], false], 8, 6], 'circle-color': '#ffffff', 'circle-stroke-color': ['case', ['boolean', ['get', 'active'], false], '#ffffff', '#94a3b8'], 'circle-stroke-width': ['case', ['boolean', ['get', 'active'], false], 2, 1], 'circle-opacity': ['case', ['boolean', ['get', 'active'], false], 1, 0.65] }} /><Layer id="maritime-node-labels" type="symbol" layout={{ 'text-field': ['get', 'shortLabel'], 'text-size': 10, 'text-offset': [0, 1.2], 'text-allow-overlap': true }} paint={{ 'text-color': '#ffffff', 'text-halo-color': '#020617', 'text-halo-width': 1 }} /></Source>
            </>
          )}
          ...
          <Source id="points" type="geojson" data={vesselPoints}><Layer id="vessel-points" type="circle" paint={{ 'circle-radius': ['case', ['==', ['get', 'mmsi'], selectedMmsi || 0], 6, 4], 'circle-color': ['case', ['in', ['get', 'navigationalStatus'], ['literal', [1, 5]]], '#f59e0b', '#10b981'], 'circle-stroke-color': '#020617', 'circle-stroke-width': 1.2, 'circle-opacity': 0.95 }} /></Source>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-8 bg-intel-panel border-t border-white/10 flex items-center px-4 justify-between text-xs text-intel-text z-50 overflow-hidden shrink-0 font-mono">
        <div className="flex space-x-6 items-center flex-1">
          ...
          <span title={feed.detail} className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${feed.className}`}>{feed.label}</span>
          <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${activeMaritimeNodeIds.length ? 'bg-cyan-500/15 text-cyan-200 border border-cyan-500/25' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>{activeMaritimeNodeIds.length ? `ACTIVE NODES: ${activeMaritimeNodeIds.length}` : 'NO NODES AVAILABLE'}</span>
          {feedStatus && <span className="text-white/35 uppercase tracking-wide">MAIN CACHE: {feedStatus.vesselCount} | MSGS: {feedStatus.totalMessagesReceived}</span>}
        </div>
