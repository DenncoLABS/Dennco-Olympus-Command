...
export function maritimeFeedHealth(status: MaritimeFeedStatus | null, isError: boolean) {
  ...
  if (status.totalMessagesReceived <= 0) return { label: 'NO_AIS_MESSAGES', className: amber, detail: 'AISStream is connected, but no AIS messages have been received yet.' };
