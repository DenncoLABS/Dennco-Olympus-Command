...
export const MaritimeRightDrawer: React.FC<MaritimeRightDrawerProps> = ({ vessel, onClose }) => {
  ...
  return (
    <DraggableDockPanel
      storageKey="olympus.maritime.detailPanel.position.v1"
      title="TARGET // MARITIME"
      subtitle="Drag to dock anywhere on the map"
      width={384}
      minHeight={420}
      defaultPosition={{ x: 16, y: 86 }}
      onClose={onClose}
      className="!border-y-0 !border-l-intel-accent/30"
    ...
    >
      ...
      <div className="pt-3 border-t border-white/10">
        <button disabled className="w-full flex items-center justify-center space-x-2 py-2 bg-intel-accent/10 hover:bg-intel-accent/20 border border-intel-accent/50 text-intel-accent transition-colors opacity-50 cursor-not-allowed"><Share2 size={14} /><span className="text-xs tracking-wider">SHARE</span></button>
      </div>
    </DraggableDockPanel>
  );
};
