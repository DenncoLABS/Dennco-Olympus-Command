...
function emergencyLabel(value: unknown) {
  ...
  if (code === 'none' || code === '') return 'SIMULATED ALERT';
...
type PrimaryTarget = { type: 'aircraft'; feature: MapFeature; distance: number } | { type: 'radar'; region: RadarRegionPin; distance: number };
...
function queryBox(point: MapPoint, radius: number): [[number, number], [number, number]] {
  return [[point.x - radius, point.y - radius], [point.x + radius, point.y + radius]];
}

function distanceToLngLat(map: import('maplibre-gl').Map, point: MapPoint, lon: number, lat: number): number {
...
function pointDistance(map: import('maplibre-gl').Map, point: MapPoint, feature: MapFeature): number {
  ...
  return distanceToLngLat(map, point, coordinates[0], coordinates[1]);
}
...
function scoreAircraftFeature(map: import('maplibre-gl').Map, point: MapPoint, feature: MapFeature) {
  ...
  return { feature, icao24: String(icao24).trim().toLowerCase(), distance: pointDistance(map, point, feature), visibleSymbol: feature.layer?.id === 'aircraft-points' };
}

function nearestAircraftTarget(map: import('maplibre-gl').Map, point: MapPoint, eventFeatures: MapFeature[]): PrimaryTarget | null {
  const radius = aircraftHitRadiusForZoom(map.getZoom());
  const pointHits = map.queryRenderedFeatures(queryBox(point, radius), { layers: AIRCRAFT_POINT_LAYERS });
  const clickHits = map.queryRenderedFeatures(queryBox(point, radius), { layers: AIRCRAFT_CLICK_LAYERS });
  ...
  for (const feature of [...pointHits, ...clickHits, ...eventFeatures]) {
    ...
    if (!existing || hit.distance < existing.distance || (hit.visibleSymbol && !existing.visibleSymbol)) bestByIcao.set(hit.icao24, hit);
  }
  const best = [...bestByIcao.values()].sort((a, b) => a.distance - b.distance || Number(b.visibleSymbol) - Number(a.visibleSymbol))[0];
  return best ? { type: 'aircraft', feature: best.feature, distance: best.distance } : null;
}

function nearestRadarTarget(map: import('maplibre-gl').Map, point: MapPoint): PrimaryTarget | null {
  const best = RADAR_REGIONS
    .map((region) => ({ region, distance: distanceToLngLat(map, point, region.lon, region.lat) }))
    .filter((hit) => hit.distance <= RADAR_HIT_RADIUS_PX)
    .sort((a, b) => a.distance - b.distance)[0];
  return best ? { type: 'radar', region: best.region, distance: best.distance } : null;
}
...
function chooseNearestPrimaryTarget(map: import('maplibre-gl').Map, point: MapPoint, features: MapFeature[]): PrimaryTarget | null {
  const aircraftTarget = nearestAircraftTarget(map, point, features);
  const radarTarget = nearestRadarTarget(map, point);
  if (aircraftTarget && radarTarget) return aircraftTarget.distance <= radarTarget.distance ? aircraftTarget : radarTarget;
  return aircraftTarget || radarTarget;
}
...
export const FlightsPage: React.FC = () => {
  ...
  const reportSelectedAirEmergency = useCallback(() => {
    ...
    setFlightAlerts((current) => [{ id: `manual-${Date.now()}`, timestamp: Date.now(), title: 'Flight Alert Reported', details: `Alert reported for ${selectedFlight.callsign || selectedFlight.registration || selectedFlight.icao24}.`, icao24: selectedFlight.icao24, callsign: selectedFlight.callsign, emergency: emergencyLabel(selectedFlight.emergency), reportedBy: 'Olympus / Local Admin user', activeDistress: true }, ...current].slice(0, 14));
  }, [selectedFlight]);
  ...
  const standDownSelectedEmergency = useCallback(() => {
    ...
    setSimulatedEmergencyIcaos((current) => { const next = new Set(current); next.delete(selectedDisplayFlight.icao24); return next; });
  ...
  const onClick = useCallback((event: import('maplibre-gl').MapMouseEvent & { features?: MapFeature[] }) => {
    const target = chooseNearestPrimaryTarget(event.target, event.point, event.features || []);
    ...
    if (target?.type === 'radar') {
      toggleRadarRegion(target.region.id);
      setInfrastructurePopup({ type: 'radar', item: target.region });
      return;
    }
    const airportFeature = featureInLayer(event.features || [], 'airport-points');
  ...
  return (
    <div className="absolute inset-0 bg-intel-bg overflow-hidden flex flex-col">
      ...
      <div ref={mapContainerRef} className="relative flex-1 min-h-0">
        <Map initialViewState={{ latitude: 39.8283, longitude: -98.5795, zoom: 4 }} mapStyle={activeMapStyle} styleDiffing={false} interactiveLayerIds={['aircraft-click-target', 'aircraft-points', 'radar-region-points', 'airport-points', 'flight-airbase-points']} onClick={onClick} cursor={selectedIcao24 ? 'pointer' : 'crosshair'} onLoad={(event: { target: import('maplibre-gl').Map }) => { mapRef.current = event.target; addPlaneImages(event.target); event.target.resize(); }} onStyleData={(event: { target: import('maplibre-gl').Map }) => { mapRef.current = event.target; addPlaneImages(event.target); event.target.resize(); }} onStyleImageMissing={(event: { id: string; target: import('maplibre-gl').Map }) => { if (event.id.startsWith('olympus-plane')) addPlaneImages(event.target); }} projection={mapProjection === 'globe' ? ({ type: 'globe' } as import('maplibre-gl').ProjectionSpecification) : ({ type: 'mercator' } as import('maplibre-gl').ProjectionSpecification)} style={{ width: '100%', height: '100%' }}>
