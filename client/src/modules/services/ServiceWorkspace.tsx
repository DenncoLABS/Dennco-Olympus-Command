    ...
    <div className="absolute inset-0 overflow-hidden bg-[#020617] font-mono text-white">
      <div className="absolute left-4 right-4 top-4 z-[20] flex items-center justify-between rounded border border-cyan-300/20 bg-black/70 px-3 py-2 backdrop-blur">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {isProxmox ? (
            <button onClick={() => open('PVE Native GUI', 'web')} className="rounded border border-emerald-300/35 bg-emerald-300/10 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-emerald-100">Native GUI</button>
          ) : null}
          <input value={url} onChange={(event) => setUrl(event.target.value)} placeholder={isProxmox ? 'https://proxmox-host:8006' : 'Real console URL'} className="w-64 rounded border border-white/10 bg-black/55 px-2 py-1 text-[10px] text-cyan-100 outline-none focus:border-cyan-300/50" />
...
function WindowBody({ isProxmox, win, url }: { isProxmox: boolean; win: Win; url: string }) {
  if (isProxmox) return <ProxmoxNativeGui kind={win.kind} url={url} />;
...
function ProxmoxNativeGui({ kind, url }: { kind: WinKind; url: string }) {
  const [reloadKey, setReloadKey] = useState(0);
  const proxmoxUrl = normalizeUrl(url);
    ...
    return (
      <div className="flex h-[calc(100%-36px)] items-center justify-center p-8 text-center">
        <div className="max-w-xl rounded border border-cyan-300/20 bg-white/[0.03] p-6">
          <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-300">Native Proxmox 8 GUI</div>
          <div className="mt-3 text-sm leading-relaxed text-white/60">Enter the real Proxmox 8 web address in the toolbar, usually https://HOST:8006, then press Save URL and reopen this window.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100%-36px)] bg-[#020617]">
      <div className="flex h-10 items-center justify-between border-b border-cyan-300/15 bg-black/70 px-3">
        <div>
          <div className="text-[10px] uppercase tracking-[0.2em] text-cyan-300">Olympus Proxmox Frame · {section}</div>
          <div className="text-[8px] uppercase tracking-[0.14em] text-white/35">Native Proxmox 8 GUI inside an Olympus workspace window</div>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => window.open(proxmoxUrl, '_blank', 'noopener,noreferrer')} className="rounded border border-white/15 px-2 py-1 text-[10px] uppercase tracking-[0.12em] text-white/60 hover:text-cyan-100">External</button>
        </div>
      </div>
      <iframe key={reloadKey} src={proxmoxUrl} className="h-[calc(100%-40px)] w-full border-0 bg-black" title="Native Proxmox 8 GUI" />
    </div>
  );
}
