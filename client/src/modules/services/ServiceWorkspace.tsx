    ...
    <div className="absolute inset-0 overflow-hidden bg-[#020617] font-mono text-white">
      <div className="absolute left-4 right-4 top-4 z-[20] flex items-center justify-between rounded border border-cyan-300/20 bg-black/70 px-3 py-2 backdrop-blur">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {isProxmox ? (
            <button onClick={() => open('PVE New Lab VM', 'vms')} className="rounded border border-emerald-300/35 bg-emerald-300/10 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-emerald-100">New Lab VM</button>
          ) : null}
      ...
      <div className="absolute inset-0 pt-16">
        ...
        {wins.map((win) => (
          <section key={win.id} className="absolute h-[520px] w-[860px] overflow-hidden rounded border border-cyan-300/20 bg-black shadow-[0_24px_60px_rgba(0,0,0,0.86)]" style={{ left: win.x, top: win.y, zIndex: win.z }}>
            ...
            <WindowBody isProxmox={isProxmox} win={win} url={url} />
          </section>
        ))}
      </div>
    </div>
...
function WindowBody({ isProxmox, win, url }: { isProxmox: boolean; win: Win; url: string }) {
  if (isProxmox) return <ProxmoxLabPanel kind={win.kind} />;
  if (win.kind === 'web' && url) return <iframe src={url} className="h-[calc(100%-36px)] w-full border-0 bg-black" title={win.title} />;
...
function ProxmoxLabPanel({ kind }: { kind: WinKind }) {
  ...
  const load = () => {
    fetch('/api/proxmox-lab/objects')
      .then((response) => response.json())
      .then((data: { objects?: LabObject[]; root?: string }) => {
  ...
  const createObject = () => {
    fetch('/api/proxmox-lab/objects', {
      ...
      body: JSON.stringify({ type: newType, id: newId, name: newName }),
    })
      .then((response) => response.json())
      .then(() => { setMessage(`created ${newType}-${newId}`); load(); })
      .catch((error: unknown) => setMessage(String(error)));
  };

  const markReady = (obj: LabObject) => {
    fetch(`/api/proxmox-lab/objects/${obj.type}/${obj.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ stage: 'ready', ready: 'true' }),
    }).then(() => load()).catch((error: unknown) => setMessage(String(error)));
  };

  if (kind === 'console') {
    return (
      <div className="h-[calc(100%-36px)] overflow-auto p-5 text-xs text-white/60">
        <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-300">Lab Console</div>
        <pre className="mt-3 rounded border border-white/10 bg-white/[0.03] p-3">{`sudo /opt/dennco/olympus-command/scripts/olympus-proxmox-lab.sh init
sudo /opt/dennco/olympus-command/scripts/olympus-proxmox-lab.sh import-qm 100 lab-vm
sudo /opt/dennco/olympus-command/scripts/olympus-proxmox-lab.sh list`}</pre>
      </div>
    );
  }
  ...
  if (kind === 'storage') {
    return (
      <div className="h-[calc(100%-36px)] overflow-auto p-5 text-sm text-white/60">
        <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-300">Lab Storage</div>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <Status label="Root" value={root || '/var/lib/dennco/olympus-lab-node/proxmox'} />
          <Status label="Objects" value="objects/qm-id.env and objects/ct-id.env" />
          <Status label="Overlays" value="overlays/type-id/rootfs and overlays/type-id/config" />
          <Status label="Mode" value="simulated QM/LXC, not live production files" />
        </div>
      </div>
    );
  }
  ...
  if (kind === 'nodes') {
    return (
      <div className="h-[calc(100%-36px)] overflow-auto p-5 text-sm text-white/60">
        <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-300">Lab Nodes</div>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <Status label="olympus-lab" value="simulated Proxmox lab node" />
          <Status label="production" value="manual-gated source" />
          <Status label="promotion" value="review plan before migration" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100%-36px)] overflow-auto p-5">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-300">Virtual Environment</div>
          <div className="text-xs text-white/40">Editable simulated QM/LXC objects under Olympus lab root.</div>
        </div>
        <button onClick={load} className="rounded border border-cyan-300/30 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-cyan-200">Refresh</button>
      </div>
      <div className="mt-4 grid gap-2 md:grid-cols-[90px_1fr_1fr_auto]">
        <select value={newType} onChange={(event) => setNewType(event.target.value as 'qm' | 'ct')} className="rounded border border-white/10 bg-black/40 px-2 py-2 text-xs text-cyan-100">
          <option value="qm">QM</option>
          <option value="ct">CT</option>
        </select>
        <input value={newId} onChange={(event) => setNewId(event.target.value)} className="rounded border border-white/10 bg-black/40 px-2 py-2 text-xs text-cyan-100" placeholder="ID" />
        <input value={newName} onChange={(event) => setNewName(event.target.value)} className="rounded border border-white/10 bg-black/40 px-2 py-2 text-xs text-cyan-100" placeholder="Name" />
        <button onClick={createObject} className="rounded border border-emerald-300/35 bg-emerald-300/10 px-3 py-2 text-[10px] uppercase tracking-[0.14em] text-emerald-100">Create</button>
      </div>
      {message ? <div className="mt-2 text-xs text-cyan-200">{message}</div> : null}
      <div className="mt-4 grid gap-3">
        {objects.map((obj) => (
          <div key={`${obj.type}-${obj.id}`} className="rounded border border-white/10 bg-white/[0.03] p-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-bold text-white">{obj.type.toUpperCase()} {obj.id} · {obj.name}</div>
                <div className="mt-1 text-xs text-white/40">{obj.overlay}</div>
              </div>
              <div className="flex gap-2">
                <span className="rounded border border-cyan-300/20 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-cyan-200">{obj.stage}</span>
                <button onClick={() => markReady(obj)} className="rounded border border-emerald-300/30 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-emerald-200">Ready</button>
              </div>
            </div>
          </div>
        ))}
...
function Status({ label, value }: { label: string; value: string }) {
  return <div className="border border-white/10 bg-white/[0.03] p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-white/35">{label}</div><div className="mt-1 break-all font-mono text-white">{value}</div></div>;
}
