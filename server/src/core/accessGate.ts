  ...
  <form method="post" action="/api/admin/setup">
    ...
    <div class="grid">
      <div>
        ...
        <input id="adminUser" name="adminUser" value="admin" autocomplete="username" />
      </div>
      <div>
        <label for="accessCode">Access code</label>
        <input id="accessCode" name="accessCode" type="password" autocomplete="new-password" autofocus />
      </div>
    </div>
    <label for="productName">Product name</label>
    <input id="productName" name="productName" value="Dennco Olympus Command" />
    <div class="grid">
      <div>
        <label for="shortName">Short name</label>
        <input id="shortName" name="shortName" value="OLYMPUS" />
      </div>
      <div>
        <label for="footerText">Footer text</label>
        <input id="footerText" name="footerText" value="Dennco Olympus Command" />
      </div>
    </div>
...
export function renderSetup(res: Response, message?: string): void {
  res.status(200).type('html').send(setupPage(message));
}

export function requireAdminAccess(req: Request, res: Response, next: NextFunction): void {
  if (
    req.path === '/health' ||
    req.path === '/api/admin/runtime-settings' ||
    req.path === '/api/admin/login' ||
    req.path === '/api/admin/logout' ||
    req.path === '/api/admin/setup' ||
    req.path === '/api/admin/setup/status'
  ) {
    return next();
  }
  ...
  if (!isAdminConfigured()) {
    if (req.path.startsWith('/api/')) {
      res.status(428).json({ error: 'First-time setup required.' });
      return;
    }
    renderSetup(res);
    return;
